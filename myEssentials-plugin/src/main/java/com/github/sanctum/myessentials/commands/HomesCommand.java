...
public class HomesCommand extends CommandInput {
	...
	public boolean onPlayer(@NotNull Player player, @NotNull String commandLabel, @NotNull String[] args) {
		MenuType.PAGINATED.build()
				...
				.setStock(i -> {
					ListElement<Warp> warps = new ListElement<>(() -> MyEssentialsAPI.getInstance().getWarpHolder(player).getAll().stream().collect(Collectors.toList()));
					warps.setLimit(7);
					warps.setPopulate((value, element) -> {
						element.setElement(ed -> ed.setTitle("&e" + value.getName()).setLore("&fClick to teleport.", "&f&m--------------------------", "&6Location: &fX: &e" + value.getLocation().getBlockX() + " &fY: &e" + value.getLocation().getBlockY() + " &fZ: &e" + value.getLocation().getBlockZ(), "&6World/Dimension: &e" + value.getLocation().getWorld().getName(), "&f&m--------------------------").setType(Material.END_CRYSTAL).build());
						element.setTypeAndAddAction(ItemElement.ControlType.DISPLAY, click -> {
							click.setCancelled(true);
							click.setHotbarAllowed(false);
							click.getElement().performCommand("home " + value.getName());
						});
					});
